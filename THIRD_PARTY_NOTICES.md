...
본 프로젝트의 자체 코드는 MIT로 표기되어 있지만, 결합 저작물(combined work) 측면에서 회색지대가 존재합니다. 핵심은 **`python-chess`(GPL-3.0)와의 결합** 해석 차이입니다.

- **강한 해석 (FSF 입장)**: GPL 모듈을 `import`하는 것 자체가 파생 저작물을 만든다고 봅니다.
- **약한 해석 (실무 관행)**: 단순 동적 import는 결합 저작물이 아니라고 봅니다.
- 두 해석의 차이는 **법원에서 명확히 판결된 적이 없는 회색지대**이며, 대부분의 오픈소스 프로젝트는 약한 해석에 따라 운영되고 있습니다.

### 시나리오별 권장

| 사용 시나리오 | 안전성 | 권장 |
|---|---|---|
| 학생 포트폴리오 / 개인 학습 | ✅ 충분히 안전 | 현재 상태 그대로 사용 |
| GitHub 공개 (현재 상태) | ✅ 일반적으로 문제없음 | 현재 상태 그대로 사용 |
| 상업적 재배포 (예: 유료 판매) | ⚠️ 주의 필요 | 전체 라이선스를 GPL-3.0으로 전환, 또는 `python-chess`를 자체 구현으로 대체 |
| 회사 제품에 통합 / 사내 배포 | ⚠️ 주의 필요 | 사내 법무 검토 후 GPL-3.0 전환 권장 |

### 정리

- 본 프로젝트의 라이선스 표기와 의존성 고지는 일반적인 오픈소스 관행에 따라 구성되어 있습니다.
- 사용 형태(개인 학습, 공개 포트폴리오, 상업적 활용 등)에 따라 적용되는 라이선스 조건이 달라질 수 있으므로, 본인의 사용 목적에 맞게 위의 시나리오 표를 참고해주세요.
- 본 안내는 법적 자문이 아니며, 실제 배포·상업화 시점에는 별도의 법무 검토를 받는 것을 권장합니다.
