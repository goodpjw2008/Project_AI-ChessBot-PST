...
| 빌드된 형태로 묶어서 배포 / 상업적 활용 | ⚠️ 주의 필요 | 전체 라이선스를 GPL-3.0으로 전환, 또는 `python-chess`를 자체 구현으로 대체 |

### 정리

- 본 프로젝트의 라이선스 표기와 의존성 고지는 일반적인 오픈소스 관행에 따라 구성되어 있습니다.
- 사용 형태(개인 학습, 공개 포트폴리오 등)에 따라 적용되는 라이선스 조건이 달라질 수 있으므로, 본인의 사용 목적에 맞게 위의 시나리오 표를 참고해주세요.
- 본 안내는 참고용이며, 추후 다른 형태로 활용하게 될 경우에는 각 의존성의 라이선스를 다시 한 번 확인해보시기 바랍니다.
